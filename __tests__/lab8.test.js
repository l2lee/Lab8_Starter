describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    console.log(`Checking product item 1 of ${prodItems.length}`);
    ...
    // TODO - Step 1
    // Right now this function is only checking the first <product-item> it found, make it so that it checks every <product-item> it found

    // Perform the above functionalities but on every other item in the list
    for (let i = 1; i < prodItems.length; i++) {
      console.log(`Checking product item ${i + 1} of ${prodItems.length}`);
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let plainValue = await innerText.jsonValue();

    // Expect the value to be "Remove from Cart"
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // self added: index starts at 1 since first item was already clicked by previous test at step 2
    for (let i = 1; i < prodItems.length; i++) {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // TODO - Step 4
    // Reload the page, then select all of the <product-item> elements, and check every element to make sure that all of their buttons say "Remove from Cart".
    await page.reload();
    let prodItems = await page.$$('product-item');
    ...
    let cartCount = await page.$('#cart-count');
    let cardCountValue = await (await cartCount.getProperty('innerText')).jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    console.log('Checking number of items in cart on screen after reload...');
    // TODO - Step 7
    // Reload the page once more, then go through each <product-item> to make sure that it has remembered nothing is in the cart - do this by checking the text on the buttons so that they should say "Add to Cart".
    await page.reload();
